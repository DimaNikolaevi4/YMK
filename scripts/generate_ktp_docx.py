...
def build_table1(doc, d):
    ...
    #  остальные графы пустые). КДЗ входит в ОБЪЁМ семестра проведения КДЗ
    # (поле kdz_semester; определяется по ведомости нагрузки — для 01.02 это с.3,
    #  для эталона 05.02 — с.4). Ячейки теории семестров — чистые занятия без КДЗ.
    att = d.get('attestation', {})
    if not att and d.get('exam_form'):
        att = {'form': d['exam_form'], 'hours': 2}
    ...
    #     объём семестра проведения КДЗ УЖЕ включает КДЗ — правило данных) ---
